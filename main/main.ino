...
// button1 = take sample - off grid button

// button2 = ISO down 2-2
// button3 = aperture down 3-2
// button4 = aperture up 3-1
// button5 = iso up 2-1
// button6 = save - grid 1-1
// button7 = mode select - grid 1-2

//button 8 = power button, monitored by A1 and power controlled by A2
...
int batinput = A0;  // battery voltage check, 100k voltage divider
int batlevel, batpercent = 0;  // battery level holder, in mV
...
//MODE ARRAY
const char* modearray[] = {"INCIDENT","REFLECT","SPOT"};
...
// text display
 ...
 // display.print(aperturearraypointer);
  ...
  display.setTextSize(2);
  display.setCursor(0,50); // first num is horz, second is vert
  ...
  else if(button2state == HIGH && button2flag == 1)
 {
   button2flag = 0;  // reset flag if button is unpressed but flag is high - ready to be pressed again
    ...
    // BUTTON 3 ACTIONS - APERTURE DOWN
   ...
   button3flag = 0;  // reset flag if button is unpressed but flag is high - ready to be pressed again
        ...
        else if(button4state == HIGH && button4flag == 1)
     {
       button4flag = 0;  // reset flag if button is unpressed but flag is high - ready to be pressed again
           ...
           // BUTTON 5 ACTIONS - ISO UP
  if(button5state == LOW && button5flag == 0 && ISOarraypointer >= 0 && ISOarraypointer <= 12) 
  {
         otc = 0; // reset OTC
    ISOarraypointer++; 
    calculations();  // recalculates the shutter/aperture combo based on new value
         button5flag = 1; // set flag high

  }
  
  else if(button5state == HIGH && button5flag == 1)
 {
   button5flag = 0;  // reset flag if button is unpressed but flag is high - ready to be pressed again
 } 
 
 
 
            // BUTTON 6 ACTIONS - SAVE
    if(button6state == LOW && button6flag == 0) 
  {
         otc = 0; // reset OTC
      // save settings
      ...
      display.setCursor(5,35); // first num is horz, second is vert
      display.setTextSize(1);
      display.println("SAVED"); 
      display.display();
      delay(300);
    
         button6flag = 1; // set flag high

  }
  
    else if(button6state == HIGH && button6flag == 1)
 {
   button6flag = 0;  // reset flag if button is unpressed but flag is high - ready to be pressed again
 } 
  
  
  
  
             // BUTTON 7 ACTIONS - MODE SELECT
  if(button7state == LOW && button7flag == 0) 
  {
         otc = 0; // reset OTC
          // cycling through modes    
         modearraypointer++;
         if(modearraypointer >= 3)
         {
           modearraypointer = 0;  // if modepointer goes out of range, put back to 0
         }
            
         button7flag = 1; // set flag high

  }
  
  else if(button7state == HIGH && button7flag == 1)
 {
   button7flag = 0;  // reset flag if button is unpressed but flag is high - ready to be pressed again
 } 
